...
<head>
    ...
    <title>Sarius War Room - Mando Supremo</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800;900&display=swap" rel="stylesheet">
    <style>
        :root{--bg:#0b0e14;--sur:#161923;--sur2:#1e2230;--brd:#2d3241;--acc:#7c6af7;--gold:#f5c542;--red:#ff4d4d;--blue:#4d94ff;--magenta:#ff00ff;--green:#4caf7d;--txt:#e8eaf6;--txt2:#a0a6c0}
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        /* GESTIÓN DE PUEBLOS */
        #village-manager{margin-top:20px; max-height: 500px; overflow-y: auto; border: 1px solid var(--brd); border-radius: 8px; background: #080a0f}
        .player-group{border-bottom: 2px solid var(--brd); padding: 10px}
        .player-name{color:var(--gold); font-weight: 800; font-size: 0.9rem; margin-bottom: 8px; display: block; background: rgba(245,197,66,0.05); padding: 5px}
        .v-row{display: grid; grid-template-columns: 1.5fr 1fr 2fr; gap: 10px; align-items: center; padding: 5px; border-bottom: 1px solid #1a1a1a}
        .v-row:hover{background: #111}
        .v-coords{font-family: monospace; color: var(--acc2)}
        .v-selector{display: flex; gap: 5px}
        .v-btn{flex:1; padding: 4px; border-radius: 4px; border: 1px solid var(--brd); background: var(--sur); color: var(--txt2); font-size: 0.6rem; font-weight: 800; cursor: pointer; text-align: center}
        ...
        .v-btn.active-front{background: #444; color: #fff; border-color: #666}

        /* OBJETIVOS */
        .obj-grid{display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px}
        .btn-main{width:100%;padding:20px;background:linear-gradient(135deg,var(--acc),var(--blue));border:none;border-radius:10px;color:#fff;font-weight:900;cursor:pointer;font-size:1.1rem;margin-top:15px;text-transform:uppercase;transition:0.3s}
...
<div class="wrap">
    <div class="header">
        ...
        <p>Mando y Control de Alianza • v4.0</p>
    </div>

    <div class="container">
        <!-- 1. ENTRADA DE DATOS DE LA TRIBU -->
        <div class="card full">
            <h2><div class="ico">💪</div> 1. CARGAR DATOS DE LA TRIBU (JSON)</h2>
            <div style="display:grid; grid-template-columns: 2fr 1fr; gap:20px">
                <textarea id="in-troops" rows="5" placeholder="Pega aquí el JSON del extractor de miembros..."></textarea>
                <div style="display:flex; flex-direction:column; gap:10px">
                    <button class="btn-main" style="margin:0; padding:10px" onclick="loadTribeData()">CARGAR JUGADORES</button>
                    <div id="load-status" style="font-size:0.7rem; color:var(--gold); font-weight:bold"></div>
                </div>
            </div>
            
            <!-- PANEL DE SELECCIÓN DE PUEBLOS -->
            <div id="village-manager-container" style="display:none; margin-top:20px">
                <label>2. ASIGNAR ROLES A LOS PUEBLOS (⚔️ OFF / 📜 Noble / 🛡️ Ignorar Front)</label>
        ...
        <div class="card full">
            ...
            <div class="obj-grid">
                <div>
                    <label style="color:var(--magenta)">🏰 CONQUISTAS (Ataque Noble)</label>
                    <textarea id="obj-conq" rows="6" placeholder="400|400"></textarea>
                </div>
                <div>
                    <label style="color:var(--red)">🔥 REALES (Limpieza OFF)</label>
                    <textarea id="obj-real" rows="6" placeholder="450|450"></textarea>
                </div>
                <div>
                    <label style="color:var(--blue)">🎭 FAKES (Distracción)</label>
        ...
        <div class="card full">
            <h2><div class="ico">⚙️</div> 4. PARÁMETROS DE TIEMPO</h2>
            ...
            <button class="btn-main" onclick="generatePlan()">🚀 LANZAR PLANIFICACIÓN SUPREMA</button>
        </div>
        ...
        <div id="sec-res" class="card full" style="display:none">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px">
                ...
                <button class="btn-main" style="width:auto; margin:0; padding:10px 20px" onclick="copyForo()">📋 COPIAR TODO PARA EL FORO</button>
            </div>
            <div style="overflow-x:auto">
                <table class="res-table">
                    <thead>
                        <tr><th>Lanzamiento</th><th>Jugador</th><th>Origen</th><th>Objetivo</th><th>Tipo</th><th>🚀</th></tr>
                    </thead>
...
<script>
    ...
    let db = []; // Base de datos de pueblos cargados
    ...
    function loadTribeData() {
        ...
        try {
            ...
            db = data.map(v => ({
                ...
                ck: `${v.x}|${v.y}`,
                vid: v.vid,
                axe: v.axe || 0, ram: v.ram || 0, snob: v.snob || 0,
                role: 'none' // 'none', 'off', 'noble', 'front'
            }));

            renderManager();
            document.getElementById('village-manager-container').style.display = "block";
            document.getElementById('load-status').innerText = `✅ ${db.length} pueblos de ${[...new Set(db.map(x=>x.p))].length} jugadores.`;
        } catch(e) { alert("Error al leer el JSON. Asegúrate de copiarlo completo."); }
    }

    function renderManager() {
        const container = document.getElementById('village-manager');
        container.innerHTML = "";
        const players = [...new Set(db.map(x => x.p))].sort();

        players.forEach(pnm => {
            let pDiv = document.createElement('div');
            pDiv.className = "player-group";
            pDiv.innerHTML = `<span class="player-name">${pnm}</span>`;
            
            db.filter(v => v.p === pnm).forEach(v => {
                let vDiv = document.createElement('div');
                vDiv.className = "v-row";
                vDiv.innerHTML = `
                    <div class="v-coords">(${v.x}|${v.y}) <small style="color:#666">🪓${v.axe} 🔨${v.ram} 📜${v.snob}</small></div>
                    <div class="v-selector" id="sel-${v.ck.replace('|','-')}">
    ...
    function generatePlan() {
        ...
        if(isNaN(arrMs)) return alert("Pon fecha y hora de llegada.");

        let results = [];
        let used = new Set();

        // 1. ASIGNAR CONQUISTAS (Solo los marcados como NOBLE)
        ...
        cT.forEach(t => {
            ...
            used.add(w.ck);
            results.push(makeOrder(w, t, 'CONQUISTA', arrMs, vM, 'snob'));
        });

        // 2. ASIGNAR REALES (Solo marcados como OFF)
        ...
        rT.forEach(t => {
            ...
            used.add(w.ck);
            results.push(makeOrder(w, t, 'REAL', arrMs, vM, uG));
        });

        // 3. FAKES (Usar cualquier cosa que no esté marcada como FRONT ni usada ya)
        let fakePool = db.filter(v => v.role !== 'front' && !used.has(v.ck));
        if(fakePool.length === 0) fakePool = db; // Fallback si no hay nada libre
        fT.forEach((t, i) => {
            const w = fakePool[i % fakePool.length];
            results.push(makeOrder(w, t, 'FAKE', arrMs, vM, uG));
        });
    ...
    function makeOrder(v, t, type, arrMs, vM, uG) {
        const d = dist(v, t);
        const travelMs = (d * SPEEDS[uG] / vM) * 60000;
        const lDate = new Date(arrMs - travelMs);
        const srv = "es100"; // Puedes hacerlo dinámico
        return {
            p: v.p, orig: v.ck, dest: t.ck, type,
            launch: lDate.toLocaleString(),
            url: `https://${srv}.guerrastribales.es/game.php?village=${v.vid}&screen=place&target=${t.vid||''}`
        };
    }
        ...
        for(let p in grouped) {
            ...
            grouped[p].forEach(r => {
                ...
                body.innerHTML += `<tr>
                    ...
                    <td>${r.orig}</td><td><b>${r.dest}</b></td>
                    <td><span class="tag ${tagCls}">${r.type}</span></td>
                    <td><button class="v-btn" style="background:var(--acc); color:#fff" onclick="window.open('${r.url}')">🚀</button></td>
                </tr>`;
            });
            mGrid.innerHTML += `<div class="msg-card"><span><b>${p}</b></span><button class="btn bm" onclick="copyMP('${p}')">📩 COPIAR MP</button></div>`;
        }
    ...
    function copyMP(p) {
        ...
        orders.forEach(o => bb += `[*]${o.type}[|]${o.launch}[|][coord]${o.orig}[/coord][|][coord]${o.dest}[/coord]\n`);
